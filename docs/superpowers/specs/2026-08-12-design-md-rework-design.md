...
| D-4 | Sizing order becomes desktop → mobile → tablet | User decision. It also resolves a live contradiction: `CLAUDE.md` said tablet-first while `AppShell` renders a `variant='permanent'` drawer with no mobile treatment. Under the new order the permanent drawer is correct and mobile is the outstanding work. |
