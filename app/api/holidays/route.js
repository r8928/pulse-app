...
import { createHoliday, listHolidays } from '../../../database.js';
import { recalculateDays } from '../../../engine/recalculate.js';
import { errorResponse } from '../../../utils/apiResponse.js';

/**
 * P-31. Each team keeps its own calendar, so two teams observe different
 * holidays on the same date (`FR-3.7`).
 *
 ...
 * it touches — the widest fan-out in the system.
 */
  ...
  try {
    ...
    if (!teamId) {
      return NextResponse.json(
        { error: 'A team is required — each team keeps its own calendar.' },
        { status: 400 },
      );
    }

    return NextResponse.json(
      await listHolidays(teamId, {
