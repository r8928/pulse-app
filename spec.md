...
| FR-3.7 | `P4` | Give each team its own holiday calendar holding typed entries (public holiday, company holiday), which `OFFICE_ADMIN` may create, edit, and soft delete. Two teams therefore observe different holidays on the same date. A calendar shall never depend on formatting or colour. Per team configuration. | Must |
| FR-3.8 | `P4` | Give each team its own weekly off pattern, so a team whose non working days are not Saturday and Sunday is supported. Per team configuration. | Must |
