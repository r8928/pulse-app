...
| ----------------- | ----- |
| FR-1.x Identity, authentication, access control | Phase 1 |
| FR-2.x Users and lifecycle | Phase 1 |
| FR-3.x Org, shifts, calendars | Phase 2 |
| FR-4.x Attendance capture | Phase 3 |
| FR-5.x Day classification | Phase 4 |
| FR-6.x Leave engine | Phase 5 |
| FR-7.x PTO and CTO | Phase 5 |
| FR-8.x Reporting and self service | Phase 6 |
| FR-9.x Audit | Phase 1, ongoing |
| NFR-x | Ongoing |

**Deferred to Phase 2.** In scope for the product, not for the MVP. The schema supports them from day one so no migration is needed when they ship: employee self service leave requests with the manager approval workflow that acts on them (`FR-6.7`), and multi company or multi tenant support.

---

## 3. Functional Requirements

Two notes on reading this section. **`FR-6.4` is the single list of everything that is configuration rather than code**; other requirements name the item and cite `FR-6.4` instead of repeating it. And where a requirement names `IT` or `MANAGER` as the actor, that says who normally does the work; it never excludes `OFFICE_ADMIN`, who holds every permission any other role holds (`FR-1.3`).
...
| FR-9.4 | Let `OFFICE_ADMIN` view the full change history of any single punch, attendance day, or leave balance. | Should |
...
| ---- | ----- | --------- |
| BR-1 | Per team | Required daily duration is seeded at 9 hours on every shift. A team working a different day length changes it on its own shift. |
| BR-2 | Per team | Shift windows are seeded at 09:00 to 18:00 and 10:00 to 19:00. Each team's default shift is chosen from the shifts defined for it; neither window is a company wide default. |
| BR-3 | Per team | The GC team works a night shift providing night support. Its window is that team's shift configuration, seeded from the old workbook's 19:00 to 04:00 annotation and confirmed by `OFFICE_ADMIN` at team setup per `FR-3.13`. |
| BR-4 | Per team | The Sales and Marketing team works a night shift on the United States Pacific timezone. The timezone is part of that team's shift definition; there is no company wide default timezone, per `FR-3.10`. |
| BR-5 | Per team | A day where less than 89 percent of the required daily duration was clocked is counted as a short day for reporting. On a 9 hour shift that is fewer than 8 hours. |

**Grace and punctuality**

| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-6 | Behaviour | A check in is compliant if it occurs at or before the shift start plus the grace period. |
| BR-7 | Per team | The grace period is seeded at 30 minutes and is held on the shift, so it is set per team. Pulse computes grace relative to the user's own shift, unlike the old workbook, which hardcoded `<= 10:30` and so was wrong for the 09:00 shift. |
| BR-8 | Behaviour | `OFFICE_ADMIN` may override a late arrival, which then counts as compliant and waives the deduction. This is an `OFFICE_ADMIN` action under `FR-6.10`, not a manager action. |
...
| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-9 | Behaviour | Deduction is determined by the worst applicable band of either the lateness test or the hours clocked test. |
| BR-11 | Behaviour | A full day of `LEAVE` deducts 1 day of the type stated on it. |

**Leave entitlement and accrual**

| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-12 | Per team | Annual entitlement is 10 Annual, 10 Sick, and 10 Casual leaves within a year — 30 typed days, credited at the start of the leave year and prorated for a joiner per `FR-2.7`. |
| BR-13 | Per team | **Accrual period and carry forward are set per team**, seeded as the leave year: the whole entitlement of `BR-12` is credited at its start. Where a team accrues over a shorter period instead, the per period figure is derived at calculation time from that team's annual entitlement and accrual period, never carried as a constant, so changing the entitlement changes the accrual with no code change. The old workbook's **single pooled balance**, accruing `+ 1.67` per month plus a balance brought forward, is not adopted at any team: Pulse holds the typed balances of `BR-12`. |
| BR-14 | Per team | **Which leave types appear in this formula is set per team**, per `BR-12` and the automatic deduction type of `FR-6.3`. Balance of a leave type is `opening balance - leaves availed of that type - automatic deductions posted to it + CTO applied against those deductions`. The PTO balance, separately, is `approved awards - PTO taken as leave - CTO applications - expiries`. The arithmetic itself is the same everywhere, and Pulse derives both at query time by replaying the ledger rather than storing either as a column, per `FR-6.8`. The old workbook's `+ H` paternity and maternity term does not carry over: Pulse never deducts those days, so they post to their own typed balance and the standard balance never moves. |
| BR-15 | Per team | **Each team keeps its own holiday calendar**, per `FR-3.7`, so two teams observe different holidays on the same date. Vacations are fixed: a holiday belongs to the team's calendar, set in advance, and is never chosen per employee. A calendar may still be corrected mid year, for example when a public holiday is announced or moved. Such a correction is audited and triggers recalculation of the affected dates. |

**Work from home**

| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-16 | Per team | The authorised work from home quota is 5 days per month, with balance carried and reported. |
| BR-17 | Per team | **Whether a team allows work from home at all, and how much, is that team's quota** per `BR-16`; a team set to zero has none. Where it is allowed, a work from home day is a working day and consumes no leave. |

**PTO, earned for extra work**

| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-18 | Per team | Half an extra working day earns 0.5 PTO. |
| BR-19 | Per team | One full extra working day earns 1 PTO, which is one day of paid leave. |
| BR-20 | Per team | One full night worked, followed by working the next working day as well, earns 2 PTO. |
| BR-21 | Per team | Teams that routinely work beyond their scheduled hours, specifically night support and the Product Owners team, take PTO in return for those hours. |

**CTO, applied against lateness**

| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-22 | Per team | Late by 22 percent up to under 44 percent of the scheduled shift: 0.25 CTO is applied, if the user has the PTO to pay for it per `BR-26`. On a 9 hour shift that is 2 hours up to under 4. |
| BR-23 | Per team | Late by 44 percent up to under 67 percent: 0.5 CTO is applied, if the user has the PTO to pay for it per `BR-26`. On a 9 hour shift that is 4 hours up to under 6. |
| BR-24 | Per team | Late by 67 percent or more, having attended: 0.75 CTO is applied, if the user has the PTO to pay for it per `BR-26`. On a 9 hour shift that is 6 hours or more. |
| BR-25 | Per team | Did not attend at all: 1 CTO is applied, if the user has the PTO to pay for it per `BR-26`, and no leave is deducted. |
| BR-26 | Per team | **Applying CTO spends PTO.** CTO is not a balance of its own, so a CTO application is paid for out of the user's unexpired PTO. Before approving one, the system checks the user has enough. If they do, that PTO is spent and the day's leave deduction is cancelled. If they do not, the application is blocked, the deduction stands, and it comes out of **the leave type that team uses for automatic deductions**, which is per team configuration seeded to Casual. `OFFICE_ADMIN` may override the block and approve anyway, per `FR-6.10`, and the override is audited. |

**Holiday work**

| Rule | Scope | Statement |
| ---- | ----- | --------- |
| BR-27 | Per team | A day counts as worked on a holiday when the user clocked more than 22 percent of the scheduled shift on a designated non working day. On a 9 hour shift that is more than 2 hours. This threshold decides the `FR-5.6` count only; the day's status is `HOLIDAY_WORK` whenever there are any punches at all, per `FR-5.9`. |
...
These are the constraints the design must respect. Each is a consequence of the requirements cited, not a new requirement.
